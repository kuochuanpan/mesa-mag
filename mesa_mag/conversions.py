...
def get_absolute_magnitude(sim_path, filter, history_file='history.data', filter_path=None):
    ...
    age, abs_mag = get_absolute_magnitude_from_raw(age, log_L, log_R, log_Teff, filter, filter_path)

    return age, abs_mag

def get_absolute_magnitude_from_raw(age, log_L, log_R, log_Teff, filter, filter_path=None):
    """
    Calculate the absolute magnitude of the star.
    ...
    ref_flux = np.trapezoid(f0*c/wave_length**2*sensitivity, x=wave_length)

    abs_mag = np.zeros_like(age)

    for i in range(len(age)):
        flux = np.trapezoid(np.pi*black_body(temperature[i])*sensitivity, wave_length)
